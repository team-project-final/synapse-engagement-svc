...
import io.synapse.community.group.member.domain.model.exception.MemberPermissionDeniedException;
import io.synapse.community.group.member.domain.model.exception.OwnerCannotLeaveException;
...
public class GroupMember {
    ...
    private GroupMember(UUID groupId, UUID userId, MemberRole role, MemberStatus status) {
        ...
        this.joinedAt = status == MemberStatus.ACTIVE ? now : null;
        this.createdAt = now;
        this.updatedAt = now;
    }
    ...
    public static GroupMember owner(UUID groupId, UUID userId) {
        ...
        return new GroupMember(groupId, userId, MemberRole.OWNER, MemberStatus.ACTIVE);
    }

    public static GroupMember invited(UUID groupId, UUID userId) {
        return new GroupMember(groupId, userId, MemberRole.MEMBER, MemberStatus.ACTIVE);
    }
    ...
    public static GroupMember joined(UUID groupId, UUID userId, boolean publicGroup) {
        ...
        return new GroupMember(
                ...
                userId,
                MemberRole.MEMBER,
                publicGroup ? MemberStatus.ACTIVE : MemberStatus.PENDING);
    }
    ...
    public void reactivate(boolean publicGroup) {
        ...
        status = publicGroup ? MemberStatus.ACTIVE : MemberStatus.PENDING;
    ...
    public boolean canManageMembers() {
        return isActive() && (role == MemberRole.OWNER || role == MemberRole.ADMIN);
    }

    public boolean isOwner() {
        return isActive() && role == MemberRole.OWNER;
    }
