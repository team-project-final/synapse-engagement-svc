# PRD: Week 3 — 기능 고도화 + 검색 RRF + AI 자동 생성
...
| 목표 | gamification 완성 / 커뮤니티 멤버십 고도화 / 검색 RRF / AI 카드 자동 생성 안정화 |
...
|----|------------|-----------|----------|
| FR-TL-201 | W4 이벤트 연동을 위한 토픽/스키마 초안이 확정된다 | 토픽명, payload 필드, 소유자, W4 검증 계획 문서화 | P0 |
...
### 2.3 @engagement-owner — Gamification 완성 + 멤버십 고도화

| ID | 유저 스토리 | 수용 기준 | 우선순위 |
|----|------------|-----------|----------|
| FR-EG-201 ✅ | 사용자가 배지를 획득할 수 있다 | 조건 달성 → 배지 수여 + 축하 모달 트리거 | P0 |
| FR-EG-202 ✅ | 사용자의 레벨이 XP 누적에 따라 자동 상승한다 | XP 임계값 도달 → 레벨업 상태가 profile/leaderboard에 반영 | P0 |
...
| FR-EG-205 ✅ | 그룹 초대/가입 신청 워크플로우가 완성된다 | 초대 수락/거절 + 가입 신청 목록/승인/거절 API 동작 | P0 |
...
| ID | 유저 스토리 | 수용 기준 | 우선순위 |
|----|------------|-----------|----------|
| FR-LC-201 | 시스템이 복습 대상 카드가 있으면 리마인더 후보를 생성한다 | 매일 스케줄러 → 복습 대상 사용자 목록 산출 → W4 이벤트 연동 계약 준비 | P0 |
...
| ID | 유저 스토리 | 수용 기준 | 우선순위 |
|----|------------|-----------|----------|
| FR-LA-201 | 노트 생성 시 AI가 자동으로 플래시카드를 생성한다 (W2 구현분 안정화) | 노트 생성 요청/내부 연동 → LLM → Card 생성 → learning-card API 호출 + 에러 처리 | P0 |
...
| ID | 항목 | 기준 |
|----|------|------|
| NFR-201 | 멤버십 워크플로우 응답 | 초대/가입 신청 처리 P95 < 200ms |
...
| From | To | 내용 | 시점 |
|------|-----|------|------|
| @learning-card-owner | @engagement-owner | 복습 완료 결과 → XP 적립 API/내부 호출 계약 안정화 | W3 Day 1~ |
| @engagement-owner | (W4 platform) | gamification.level.up/badge.earned 이벤트 계약 초안 제공 | W3 종료 시점 |
| @learning-card-owner | (W4 platform) | 복습 완료/리마인더 이벤트 계약 초안 제공 | W3 종료 시점 |
...
- [x] 커뮤니티 초대 수락/거절 + 가입 신청 관리 API 동작
...
| W4 이벤트 계약 미확정 | W4 통합 지연 | 중 | W3 종료 시점에 필드/토픽/소유자 초안만 확정하고 구현은 W4에서 진행 |
