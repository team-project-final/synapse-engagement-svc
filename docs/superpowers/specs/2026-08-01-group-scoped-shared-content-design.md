...
- **fork한 사본은 공개 검색에 노출된다(최종 리뷰 I-1, 수용된 위험).** 이 모델에서 `group_id IS NULL`은 "개인 소유"가 아니라 "공개 검색 대상 + 익명 열람 가능"을 뜻한다. 따라서 그룹 전용 공유를 ACTIVE 멤버가 fork하면, 제목·설명·태그가 그대로인 사본이 `GET /api/v1/community/search`(permitAll)에 노출된다. 멤버 입장에서 fork 버튼이 공개 게시라는 신호는 없다. 근본 해결은 `SharedContent`에 "공개 / 그룹 / 비공개" 3상태 `visibility` 개념을 도입하는 것이며, 이번 범위 밖으로 두고 후속 과제로 남긴다. 그때까지 프론트 fork UI에는 "사본은 공개로 전환됩니다" 취지의 안내가 필요하다.
- **"그룹 전용"은 기밀성 보장이 아니다(최종 리뷰 M-1).** 공개 그룹(`isPublic`)은 가입 즉시 멤버가 `ACTIVE`가 되므로(`MemberService`의 `GroupMember.joined(group, userId, group.isPublicGroup())`), 인증된 사용자라면 누구나 가입 한 번으로 그 그룹의 전용 콘텐츠를 열람할 수 있다. 이 기능이 주는 것은 "임의의 익명 사용자로부터의 차단"이지 "지정된 사람만 열람"이 아니다. 기능 설명·UI 문구도 이 수준으로 표현해야 한다.
- **소유자도 그룹에서 나가면 자기 콘텐츠를 읽지 못한다(최종 리뷰 M-2).** 가시성 검사에 소유자 예외가 없어, 작성자가 `KICKED`가 되면 자기가 만든 그룹 공유물에 대해 `GET /share/{token}`과 fork가 404가 된다. 반면 삭제는 소유자 권한이라 계속 성공한다 — 읽기와 삭제의 권한이 어긋난다. 현재 권한표대로의 동작이지만 실사용에서는 버그로 읽힐 수 있어, 소유자 예외 도입 여부를 후속으로 판단한다.
